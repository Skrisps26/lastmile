...
function AdminView({ liveOrders, onExport, onNewOrder }: { liveOrders: LiveOrder[]; onExport: () => void; onNewOrder: () => void }) {
  const [filter, setFilter] = useState('All orders');
  const tableOrders = liveOrders;
  useEffect(() => { const button = Array.from(document.querySelectorAll('button')).find((item) => item.textContent?.includes('New order')); if (!button) return; button.addEventListener('click', onNewOrder); return () => button.removeEventListener('click', onNewOrder); }, [onNewOrder]);
...
function ConnectedShipmentModal({ onClose, onCreated, adminMode = false }: { onClose: () => void; onCreated: (order: any) => void; adminMode?: boolean }) {
  ...
  useEffect(() => { if (adminMode) fetch('/api/customers').then((response) => response.json()).then((data) => { setCustomers(data.customers || []); setCustomerId(data.customers?.[0]?.id || ''); }).catch(() => setError('Unable to load customers')); }, [adminMode]);
  const update = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value }));
  const submit = async () => {
    setSaving(true); setError('');
    try {
      const payload = { ...form, packageLengthCm: Number(form.packageLengthCm), packageBreadthCm: Number(form.packageBreadthCm), packageHeightCm: Number(form.packageHeightCm), actualWeightKg: Number(form.actualWeightKg), isCod: false, codAmount: 0, declaredValue: 0, customerType: 'B2C', ...(adminMode && customerId ? { customerId } : {}) };
  ...
  const field = (key: string, label: string, placeholder: string) => <>{adminMode && key === 'recipientName' && <label>Customer<select value={customerId} onChange={(event) => setCustomerId(event.target.value)}><option value="">Select a customer</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name} · {customer.email}</option>)}</select></label>}<label>{label}<input value={(form as any)[key]} onChange={(event) => update(key, event.target.value)} placeholder={placeholder} /></label></>;
...
function FunctionalAgentView({ liveOrders }: { liveOrders: LiveOrder[] }) {
  const [agent, setAgent] = useState<any>(null);
  const [ordersForAgent, setOrdersForAgent] = useState(liveOrders);
  const [busy, setBusy] = useState<string | null>(null);
  const [message, setMessage] = useState('');
  useEffect(() => { setOrdersForAgent(liveOrders); }, [liveOrders]);
  useEffect(() => { fetch('/api/auth/me').then((response) => response.json()).then((data) => setAgent(data.user?.agentProfile || null)).catch(() => undefined); }, []);
  const nextStatus: Record<string, string> = { ASSIGNED: 'PICKED_UP', PICKED_UP: 'IN_TRANSIT', IN_TRANSIT: 'OUT_FOR_DELIVERY', OUT_FOR_DELIVERY: 'DELIVERED', RESCHEDULED: 'OUT_FOR_DELIVERY' };
  const advance = async (order: LiveOrder) => {
    const current = order.raw?.currentStatus || String(order.status).toUpperCase().replaceAll(' ', '_');
    const next = nextStatus[current];
    if (!next) return;
    setBusy(order.id); setMessage('');
    try {
      const response = await fetch(`/api/orders/${order.id}/status`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: next }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Unable to update status');
      setOrdersForAgent((currentOrders) => currentOrders.map((item) => item.id === order.id ? { ...item, status: next.replaceAll('_', ' ').replace(/\b\w/g, (char) => char.toUpperCase()), raw: { ...item.raw, currentStatus: next } } : item));
      setMessage(`${order.trackingNumber} is now ${next.replaceAll('_', ' ').toLowerCase()}.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Unable to update status'); } finally { setBusy(null); }
  };
  const toggleAvailability = async () => { if (!agent) return; const status = agent.status === 'AVAILABLE' ? 'OFFLINE' : 'AVAILABLE'; const response = await fetch(`/api/agents/${agent.id}/status`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) }); if (response.ok) setAgent({ ...agent, status }); };
  return <><div className="page-heading"><div><div className="eyebrow accent-eyebrow">Monday, 17 June 2024 · Shift 08:00–17:00</div><h1>Ready when you are, Arjun</h1><p>Your assigned route, status updates, and delivery handoffs live here.</p></div><button className="button button-primary" onClick={() => document.getElementById('orders')?.scrollIntoView({ behavior: 'smooth' })}><MapPin size={17} /> Open route</button></div><section className="agent-hero"><div><span className="hero-kicker"><span className="pulse" /> {agent?.status === 'OFFLINE' ? 'You’re offline' : 'You’re online'}</span><h2>Make today<br /><em>count.</em></h2><p>{ordersForAgent.length} assigned deliveries in your queue.</p></div><div className="route-stat"><div className="ring"><strong>{ordersForAgent.length}</strong><span>assigned</span></div><div><span className="mini-label">Availability</span><strong>{agent?.status === 'OFFLINE' ? 'Offline' : 'Available for dispatch'}</strong><button className="text-button" onClick={toggleAvailability}>{agent?.status === 'OFFLINE' ? 'Go online' : 'Go offline'} <ArrowRight size={14} /></button></div></div></section><div className="metrics-grid"><MetricCard label="Assigned deliveries" value={String(ordersForAgent.length)} icon={Truck} accent="violet" /><MetricCard label="Completed today" value={String(ordersForAgent.filter((order) => order.status === 'Delivered').length)} icon={Check} accent="teal" /><MetricCard label="Agent status" value={agent?.status === 'OFFLINE' ? 'Offline' : 'Ready'} icon={Clock3} accent="orange" /><MetricCard label="Tracking IDs" value={String(ordersForAgent.length)} icon={Package} accent="pink" /></div>{message && <div className="action-message">{message}</div>}<section className="panel deliveries-panel" id="orders"><div className="panel-heading"><div><h2>Assigned deliveries</h2><span>Dispatch assigns orders; you move them forward</span></div><span className="dispatch-badge"><ShieldCheck size={13} /> Dispatch controlled</span></div>{ordersForAgent.length ? <div className="delivery-table"><div className="table-head"><span>Tracking ID</span><span>Drop-off</span><span>Customer</span><span>Status</span><span>Next action</span><span /></div>{ordersForAgent.map((order) => { const current = order.raw?.currentStatus || String(order.status).toUpperCase().replaceAll(' ', '_'); const next = nextStatus[current]; return <div className="table-row" key={order.id}><div className="shipment-cell"><span className={`order-icon ${order.tone}`}><Package size={16} /></span><strong>{order.trackingNumber}</strong></div><span>{order.raw?.recipientStreet || order.route}</span><span>{order.raw?.recipientName || 'Customer'}</span><StatusPill status={order.status} /><button className="row-action" disabled={!next || busy === order.id} onClick={() => advance(order)}>{busy === order.id ? 'Saving…' : next ? `Mark ${next.replaceAll('_', ' ').toLowerCase()}` : 'Complete'}</button><span /></div>})}</div> : <div className="empty-state agent-empty"><Truck size={25} /><strong>No deliveries assigned yet</strong><span>Unassigned orders are handled by admin dispatch or auto-assignment. You’ll see them here once assigned.</span><button className="button button-primary" onClick={toggleAvailability}>{agent?.status === 'OFFLINE' ? 'Go online for dispatch' : 'Update availability'}</button></div>}</section></>;
}
...
export function Dashboard() {
  ...
  const content = role === 'customer' ? <CustomerView customerOrders={liveOrders} onCreate={() => setShowCreate(true)} /> : role === 'agent' ? <FunctionalAgentView liveOrders={liveOrders} /> : <AdminView liveOrders={liveOrders} onExport={exportOrders} onNewOrder={() => setShowAdminCreate(true)} />;
  return <div className="app-shell"><Sidebar role={role} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} onLogout={handleLogout} /><div className="app-main"><Topbar role={role} setMobileOpen={setMobileOpen} onLogout={handleLogout} /><main className="main-content">{content}</main><footer className="app-footer"><span>lastmile<span className="brand-dot">.</span> operations platform</span><span>All systems operational <span className="pulse" /></span></footer></div>{showCreate && <ConnectedShipmentModal onCreated={addOrder} onClose={() => setShowCreate(false)} />}{showAdminCreate && <ConnectedShipmentModal adminMode onCreated={addOrder} onClose={() => setShowAdminCreate(false)} />}</div>;
}
